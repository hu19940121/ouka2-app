...
1. **启动应用** - 双击桌面图标或从开始菜单启动「欧卡2中国电台」

2. **获取电台列表** - 首次启动时，点击「刷新数据」按钮获取最新的电台列表
   
3. **启动流媒体服务器** - 点击「启动服务器」按钮
   - 服务器会在本地 `3000` 端口运行
   - 这是让欧卡2能够收听电台的关键步骤

4. **试听电台**（可选）- 点击任意电台卡片上的播放按钮，确认电台能正常播放

5. **安装到欧卡2** - 点击「安装到欧卡2」按钮
   - 应用会自动在以下位置生成 `live_streams.sii` 配置文件：
   - `C:\Users\你的用户名\Documents\Euro Truck Simulator 2\`

6. **进入游戏** - 启动欧卡2，在卡车收音机中就能看到中国电台了！
